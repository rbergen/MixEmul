...
namespace MixAssembler.Instruction
{
	...
	{
		private IValue mAddress;
		private IValue mField;
		private int mFieldPartCharIndex;
		private IValue mIndex;
		private int mIndexPartCharIndex;
		...
		public InstructionInstanceBase CreateInstance(InstructionBase instruction, AssemblingStatus status)
		{
			...
			{
				throw new ArgumentException("instruction must be a MixInstruction", "instruction");
			}

			MixInstruction mixInstruction = (MixInstruction)instruction;
			...
			Word word = new Word(MixInstruction.AddressByteCount);
			...
			FullWord instructionWord = new FullWord();
